...
- A customized AWS CloudWatch dashboard specifically designed monitoring all your FSx for ONTAP file systems in a single region on one dashboard.
- Management of CloudWatch alarms.
- Capture of EMS message from FSxN file systems and send them to CloudWatch Logs.

The CloudFormation template provides for three different deployment options:
...
### CloudWatch Alarms
The solution automatically creates CloudWatch alarms for the following FSxN components:
- CPU Utilization
...
2. Lambda function - The service does the following:
    ...
    1. Collects metrics directly from ONTAP (like snapmirror health status).
    1. Create CloudWatch alarms for various components of all the FSxN file systems in the region.
    1. Collects EMS message directly fron ONTAP 
